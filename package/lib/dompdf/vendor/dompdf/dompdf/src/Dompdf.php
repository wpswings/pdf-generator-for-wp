...
class Dompdf {
	...
	public function loadHtmlFile( $file, $encoding = null ) {
		...
		array( $contents, $http_response_header ) = Helpers::getFileContent( $uri, $this->httpContext );
		if ( empty( $contents ) ) {
			throw new Exception( "File '$file' not found." );
		}
	...
	private function processHtml() {
		...
		if ( $base_nodes->length && ( $href = $base_nodes->item( 0 )->getAttribute( 'href' ) ) ) {
			array( $this->protocol, $this->baseHost, $this->basePath ) = Helpers::explode_url( $href );
		}
