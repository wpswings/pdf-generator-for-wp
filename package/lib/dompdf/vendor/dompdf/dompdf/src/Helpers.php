...
class Helpers {
	...
	public static function dompdf_getimagesize( $filename, $context = null ) {
		...
		array( $width, $height, $type ) = getimagesize( $filename );
		...
		if ( $width == null || $height == null ) {
			array( $data, $headers ) = self::getFileContent( $filename, $context );
			...
			if ( ! empty( $data ) ) {
				if ( substr( $data, 0, 2 ) === 'BM' ) {
					...
					if ( strpos( $data, '<svg' ) !== false ) {
						...
						array( $width, $height ) = $doc->getDimensions();
						$type = 'svg';
					}
				}
			}
	...
	public static function getFileContent( $uri, $context = null, $offset = 0, $maxlen = null ) {
		$content = null;
		$headers = null;
		array( $proto, $host, $path, $file ) = self::explode_url( $uri );
