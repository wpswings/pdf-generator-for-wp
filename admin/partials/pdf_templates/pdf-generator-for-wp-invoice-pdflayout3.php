	...
	if ( $order_details ) {
		$html = '<!DOCTYPE html>
            ...
            <div id="company">
              ...
              <div>'.ucfirst( $company_address ).' , '.$company_city.' ,' .$company_state.' ,'.$company_pin.'</div>
      ...
      <table style ="text-align:center;" border="0" cellspacing="0" cellpadding="0">
        ...
        foreach ( $order_product_details as $product ) {
            ...
            <tr>
                <td class="no">'.$i.'</td>
                <td  class="desc"><h3>'. $product['product_name'].$meta_data.'</h3></td>
                <td class="unit">' . wc_price( $product['product_price'] ) . '</td>
            
                <td class="qty">' . $product['product_quantity']  . '</td>
         
                <td class="total">' . wc_price( $product['product_price']* $product['product_quantity'] ). '</td>
            </tr>
         ';
         $i++;
