		...
		<ul class="mwb-navbar__items">
			...
			if ( is_array( $pgfw_default_tabs ) && ! empty( $pgfw_default_tabs ) ) {
				foreach ( $pgfw_default_tabs as $pgfw_tab_key => $pgfw_default_tabs ) {
					...
					} elseif ( 'pdf-generator-for-wordpress-pdf-setting' === $pgfw_active_tab ) {
						if ( 'pdf-generator-for-wordpress-pdf-icon-setting' === $pgfw_tab_key ) {
							$pgfw_tab_classes .= 'active';
						}
					}
					?>
...
<section class="mwb-section">
	<form action="" method="POST" class="mwb-pgfw-gen-section-form">
		<div class="pgfw-section-wrap">
			...
			?>
			<div><?php echo esc_html__( 'Add', 'pdf-generator-for-wordpress' ) . ' <b>[PGFW_GENERATE_PDF]</b> ' . esc_html__( 'shortcode anywhere on your page or posts to display PDF generating icon.', 'pdf-generator-for-wordpress' ); ?></div>
		</div>
	</form>
</section>
