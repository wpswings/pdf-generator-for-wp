 ...
 */
class Pdf_Generator_For_WordPress {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Pdf_Generator_For_Wordpress_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		...
		if ( defined( 'PDF_GENERATOR_FOR_WORDPRESS_VERSION' ) ) {

			$this->version = PDF_GENERATOR_FOR_WORDPRESS_VERSION;
		} else {
		...
		$this->pdf_generator_for_wordpress_dependencies();
		$this->pdf_generator_for_wordpress_locale();
		if ( is_admin() ) {
			$this->pdf_generator_for_wordpress_admin_hooks();
		} else {
			$this->pdf_generator_for_wordpress_public_hooks();
		}
		$this->pdf_generator_for_wordpress_common_hooks();

		$this->pdf_generator_for_wordpress_api_hooks();
	 ...
	 * @access   private
	 */
	private function pdf_generator_for_wordpress_dependencies() {

		/**
		 * The class responsible for orchestrating the actions and filters of the
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-pdf-generator-for-wordpress-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-pdf-generator-for-wordpress-i18n.php';
		...
		if ( is_admin() ) {
			...
			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-pdf-generator-for-wordpress-public.php';

		}

		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'package/rest-api/class-pdf-generator-for-wordpress-rest-api.php';
	 ...
	 * @access   private
	 */
	private function pdf_generator_for_wordpress_locale() {

		$plugin_i18n = new Pdf_Generator_For_Wordpress_I18n();
	 ...
	 * @access   private
	 */
	private function pdf_generator_for_wordpress_admin_hooks() {

		$pgfw_plugin_admin = new Pdf_Generator_For_WordPress_Admin( $this->pgfw_get_plugin_name(), $this->pgfw_get_version() );
		...
		$this->loader->add_action( 'admin_menu', $pgfw_plugin_admin, 'mwb_pgfw_remove_default_submenu', 50 );

		// All admin actions and filters after License Validation goes here.
		// Adding sub menu page.
		$this->loader->add_filter( 'mwb_add_plugins_menus_array', $pgfw_plugin_admin, 'pgfw_admin_submenu_page', 15 );
	 ...
	 * @access   private
	 */
	private function pdf_generator_for_wordpress_common_hooks() {
	 ...
	 * @access   private
	 */
	private function pdf_generator_for_wordpress_public_hooks() {

		$pgfw_plugin_public = new Pdf_Generator_For_WordPress_Public( $this->pgfw_get_plugin_name(), $this->pgfw_get_version() );
	 ...
	 * @access   private
	 */
	private function pdf_generator_for_wordpress_api_hooks() {

		$pgfw_plugin_api = new Pdf_Generator_For_Wordpress_Rest_Api( $this->pgfw_get_plugin_name(), $this->pgfw_get_version() );

		$this->loader->add_action( 'rest_api_init', $pgfw_plugin_api, 'mwb_pgfw_add_endpoint' );
	 ...
	 *
	 * @since     1.0.0
	 * @return    Pdf_Generator_For_Wordpress_Loader    Orchestrates the hooks of the plugin.
	 */
	 ...
	 *
	 * @since     1.0.0
	 * @return    Pdf_Generator_For_Wordpress_Onboard    Orchestrates the hooks of the plugin.
	 */
	...
	public function mwb_pgfw_plug_default_tabs() {
		...
		$pgfw_default_tabs['pdf-generator-for-wordpress-general'] = array(
			'title' => esc_html__( 'General Settings', 'pdf-generator-for-wordpress' ),
		...
		$pgfw_default_tabs['pdf-generator-for-wordpress-advanced'] = array(
			'title' => esc_html__( 'Advanced Settings', 'pdf-generator-for-wordpress' ),
			'name'  => 'pdf-generator-for-wordpress-advanced',
		);
	...
	public function mwb_pgfw_plug_default_sub_tabs() {
		...
		$pgfw_default_tabs['pdf-generator-for-wordpress-footer'] = array(
	 ...
	 * @since  1.0.0
	 */
	public static function mwb_pgfw_plug_admin_notice( $pgfw_message, $type = 'error' ) {
	 ...
	 * @since  1.0.0
	 */
	public function mwb_pgfw_plug_generate_html( $pgfw_components = array() ) {
			...
			foreach ( $pgfw_components as $pgfw_component ) {
				if ( ! empty( $pgfw_component['type'] ) && ! empty( $pgfw_component['id'] ) ) {
					switch ( $pgfw_component['type'] ) {
						...
						<div class="mwb-form-group">
							...
							<div class="mwb-form-group__control">
								<label class="mdc-text-field mdc-text-field--outlined mdc-text-field--with-trailing-icon">
									...
									<input 
									class="mdc-text-field__input <?php echo ( isset( $pgfw_component['class'] ) ? esc_attr( $pgfw_component['class'] ) : '' ); ?> mwb-form__password" 
									name="<?php echo ( isset( $pgfw_component['name'] ) ? esc_html( $pgfw_component['name'] ) : esc_html( $pgfw_component['id'] ) ); ?>"
									id="<?php echo esc_attr( $pgfw_component['id'] ); ?>"
									type="<?php echo esc_attr( $pgfw_component['type'] ); ?>"
									value="<?php echo ( isset( $pgfw_component['value'] ) ? esc_attr( $pgfw_component['value'] ) : '' ); ?>"
						...
						<div class="mwb-form-group">
							<div class="mwb-form-group__label">
								<label for="" class="mwb-form-label"><?php echo ( isset( $pgfw_component['title'] ) ? esc_html( $pgfw_component['title'] ) : '' ); ?></label>
							</div>
							<div class="mwb-form-group__control">
								<div>
